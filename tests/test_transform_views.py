...
def test_build_view_sql_uses_the_view_own_source_table_by_default():
    ...
    assert "`proj.ds.dim_well`" in sql


def test_build_view_sql_source_table_override():
    sql = build_view_sql(project="proj", dataset="ds", view_name="wells_view", source_table="dim_well_v2")

    assert "`proj.ds.dim_well_v2`" in sql
    assert "`proj.ds.dim_well`" not in sql
