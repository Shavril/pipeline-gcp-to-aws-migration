...
def test_build_star_schema_table_sql_leaves_no_unfilled_placeholders(table_name):
    sql = build_star_schema_table_sql(project="proj", dataset="ds", table_name=table_name)

    assert f"CREATE OR REPLACE TABLE `proj.ds.{table_name}`" in sql
    assert "{" not in sql and "}" not in sql


def test_dim_lease_references_district_and_operator_natural_keys():
    sql = build_star_schema_table_sql(project="proj", dataset="ds", table_name="dim_lease")
...
def test_dim_well_carries_its_own_district_code():
    ...
    sql = build_star_schema_table_sql(project="proj", dataset="ds", table_name="dim_well")

    assert "district_code" in sql


def test_fact_oil_production_references_lease_and_date_natural_keys():
    sql = build_star_schema_table_sql(project="proj", dataset="ds", table_name="fact_oil_production")
...
def test_build_star_schema_table_sql_unknown_table_raises():
    with pytest.raises(KeyError):
        build_star_schema_table_sql(project="proj", dataset="ds", table_name="nonexistent_table")
