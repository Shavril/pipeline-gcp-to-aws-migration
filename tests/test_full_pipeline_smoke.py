...
tapes and real GCP infrastructure.
...
nor GCP access. Run it explicitly with `pytest -m smoke`.
...
def test_full_pipeline_runs_end_to_end_against_real_infrastructure():
    ...
    oil_production_gcs_uri = _unwrap(oil_production_gcs(oil_production_parquet_path))
    lease_operators_gcs_uri = _unwrap(lease_operators_gcs(lease_operators_parquet_path))
    wells_gcs_uri = _unwrap(wells_gcs(wells_parquet_path))
    assert oil_production_gcs_uri.startswith("gs://")
    assert lease_operators_gcs_uri.startswith("gs://")
    assert wells_gcs_uri.startswith("gs://")
